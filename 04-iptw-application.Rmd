...
After weighting the Beck score remains above the recommended threshold. That is why, we suggest adjusting on it (i.e., doubly robust approach) when the final outcome is modeled [@funk_doubly_2011].
...
weighted<- survfit(Surv(TIME_TO_EVENT, EVENT)~TREAT0, data = data, weights = weights)
...
# Usage for raw and IPTW survival probabilities at week 26
unadjusted_treatment_0 <- format_survival_prob(unadjusted, 26, 1)
unadjusted_treatment_1 <- format_survival_prob(unadjusted, 26, 2)
weighted_treatment_0 <- format_survival_prob(weighted, 26, 1)
weighted_treatment_1 <- format_survival_prob(weighted, 26, 2)
...
As outlined, differences in time to relapse vary depending on the analysis and treatment group considered. In the unadjusted analysis, we observed a survival probability of `r unadjusted_treatment_0` for Sertralex, compared to `r unadjusted_treatment_1` for Duloxyn. In the weighted analysis (where weights are used to create a pseudo population), these differences were reduced, with survival probabilities of `r weighted_treatment_0` for Sertralex and `r weighted_treatment_1` for Duloxyn.

Second, unadjusted, multivariable, and weighted Cox are displayed.

```{r cox}
# Fit the Cox models
unadjusted <- coxph(Surv(TIME_TO_EVENT, EVENT) ~ TREAT0, data = data)
multivariable <- coxph(Surv(TIME_TO_EVENT, EVENT) ~ TREAT0 + AGE + GENDER + SOCIO_ECO + BECK0, data = data)
weighted <- coxph(Surv(TIME_TO_EVENT, EVENT) ~ TREAT0 + BECK0, weights = weights, data = data)

# Create regression tables
unadjusted_table <- unadjusted %>% 
  tbl_regression(exponentiate = TRUE,
                 label = list(TREAT0 ~ "Treatment (unadjusted)"))

multivariable_table <- multivariable %>% 
  tbl_regression(exponentiate = TRUE,
                 label = list(TREAT0 ~ "Treatment (multivariable regression)"))%>%
  modify_table_body(
    ~ .x %>% dplyr::filter(variable == "TREAT0")
  )
...
As shown, the treatment effect varies depending on the analysis and treatment group considered. In the raw analysis, the `r unadjusted_hr_ci`, suggesting that Duloxyn significantly increases the risk of relapse. Conversely, the multivariate model and IPTW analyses yielded an `r multivariable_hr_ci`, and `r weighted_hr_ci`, respectively, indicating a trend toward Duloxyn having a protective effect by prolonging the time to relapse, although these results were not statistically significant. Lastly, the slight differences between the multivariate and IPTW models highlight that conditional and marginal effects are not equivalent.
