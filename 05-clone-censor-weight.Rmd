...
combined_data_covariate <- combined_data %>%
  select(ID, AGE, GENDER, SOCIO_ECO, BECK)
...
ms_cens_duloxyn <- coxph(Surv(TSTART, TSTOP, CENSOR) ~ AGE + GENDER + SOCIO_ECO + BECK, 
                         ties = "efron", data = combined_data_duloxyn)

combined_data_duloxyn$weights <- 1 / predict(ms_cens_duloxyn, 
                                                       new.data = combined_data_duloxyn, 
                                                       type = "survival")
...
ms_cens_sertralex <- coxph(Surv(TSTART, TSTOP, CENSOR) ~ AGE + GENDER + SOCIO_ECO + BECK, 
                           ties = "efron", data = combined_data_sertralex)

combined_data_sertralex$weights <- 1 / predict(ms_cens_sertralex, 
                                                         new.data = combined_data_sertralex, 
                                                         type = "survival")
...
Next, the Cox model is implemented where weights are considred into the analysis.
...
As seen, the treatment effect after accounting for immortal time bias is estimated at `r ccw_hr_ci`.